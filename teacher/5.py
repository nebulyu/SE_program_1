...
class GradingApp(QMainWindow):
    ...
    def initUI(self):
        ...
        for index, entry in enumerate(self.data):
            ...
            if type_ == "TITLE":
                self.handle_title(layout, index, note, text)
            elif type_ == "INTRODUCTION":
                self.handle_intro(layout, index, note, text)
            elif type_ == "GRADE":
                self.handle_grade(layout, index, note)
            elif type_ == "COMMENT":
                self.handle_comment(layout, index, note)
    ...
    def handle_title(self, layout, index, note, text):
        note_label = QLabel(f"Title: {note}")
        text_label = QLabel(f"Text: {text}")
        layout.addWidget(note_label)
        layout.addWidget(text_label)

    def handle_intro(self, layout, index, note, text):
        note_label = QLabel(f"Intro: {note}")
        text_label = QLabel(f"Text: {text}")
        layout.addWidget(note_label)
        layout.addWidget(text_label)

    def handle_grade(self, layout, index, note):
        ...
        # 将 grade 的输入框存储到字典中
        self.inputs[index] = spin_box

    def handle_comment(self, layout, index, note):
        ...
        # 将 comment 的输入框存储到字典中
        self.inputs[index] = text_edit
    ...
    def save_to_file(self):
        ...
        with open(filename, "w") as file:
            for index, entry in enumerate(self.data):
                ...
                # 如果是 GRADE 或 COMMENT，取用户输入值
                if type_ == "GRADE":
                    value = self.inputs[index].value()  # 获取 SpinBox 的值
                    file.write(f"TYPE: {type_}, NOTE: {note}, VALUE: {value}\n")
                elif type_ == "COMMENT":
                    value = self.inputs[index].toPlainText()  # 获取 TextEdit 的值
                    file.write(f"TYPE: {type_}, NOTE: {note}, COMMENT: {value}\n")
                else:
    ...
    def load_data_from_file():
        """从.tomark文件加载数据并解析"""
        filename, _ = QFileDialog.getOpenFileName(None, "Open .tomark File", "", "Text Files (*.tomark)","Text Files (*.txt)")
        if not filename:
            return None
        try:
            data = []
