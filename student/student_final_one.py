            ...
            with open(filename, "r") as file:
                for line in file:
                    ...
                    if "TYPE: GRADE" in line:
                        ...
                        if match:
                            label = QLabel(f"Type: {match.group(1)} | Note: {match.group(2)} | Value: {match.group(3)}")
                        ...
                        if match:
                            label = QLabel(f"Type: {match.group(1)} | Note: {match.group(2)} | Comment: {match.group(3)}")
                            label.setWordWrap(True)
                            self.layout.addWidget(label)
                        ...
                        if match:
                            label = QLabel(f"Type: {match.group(1)} | Note: {match.group(2)} | Text: {match.group(3)}")
                            label.setWordWrap(True)
                            self.layout.addWidget(label)
